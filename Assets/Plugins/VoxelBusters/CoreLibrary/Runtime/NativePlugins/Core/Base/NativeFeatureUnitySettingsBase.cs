    ...
    public abstract class NativeFeatureUnitySettingsBase
    {
        ...
        public bool IsEnabled
        {
            get
            {
                ...
                {
                    Debug.LogWarning(string.Format("Resetting feature: {0} state to active because current build configuration provides limited support for stripping code. Check EasyMLKitSettings file for fix.", GetFeatureName()));
                    m_isEnabled     = true;
                }
#endif
                return m_isEnabled;
            ...
            set
            {
                ...
                {
                    Debug.LogWarning(string.Format("Ignoring feature: {0} state value change request because current build configuration provides limited support for stripping code. Check EasyMLKitSettings file for fix.", GetFeatureName()));
                    return;
                }
                m_isEnabled = value;
#endif
