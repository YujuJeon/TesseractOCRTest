...
namespace VoxelBusters.EasyMLKit.Implementations.Simulator
{
    ...
    {
        public BarcodeScannerImplementation(IInputSource source) : base(isAvailable: true)
        {
        }
        ...
        public override void Process(OnProcessUpdateInternalCallback<BarcodeScannerResult> callback)
        {
            if (callback != null)
            {
                Barcode barcode = new Barcode(BarcodeFormat.AZTEC, BarcodeValueType.UNKNOWN, "testing", "test", "Display test", new UnityEngine.Rect(37,37, 950, 950));
