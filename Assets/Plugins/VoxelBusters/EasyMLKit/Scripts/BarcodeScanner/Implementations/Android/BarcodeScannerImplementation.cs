...
{
    public class BarcodeScannerImplementation : NativeFeatureBase, IBarcodeScannerImplementation
    {
        ...
        public BarcodeScannerImplementation(IInputSource inputSource) : base(inputSource)
        {
            m_instance = new NativeGoogleMLKitBarcodeScanner(NativeUnityPluginUtility.GetContext(), GetNativeInputImageProducer(inputSource));
        }

        public void Prepare(BarcodeScannerOptions options, OnPrepareCompleteInternalCallback callback)
        {
            if (!m_initialised)
            {
                Initialise();
        ...
        private Barcode ConvertFromNativeBarcode(NativeBarcode source, NativeInputImageInfo dimensions)
        {
            ...
            return new Barcode(barcodeFormat, valueType, value, source.GetRawValue(), source.GetDisplayValue(), GetRect(source.GetBoundingBox(), dimensions));
        }
