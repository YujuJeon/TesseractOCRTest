...
{
    public class TextRecognizerImplementation : NativeFeatureBase, ITextRecognizerImplementation
    {
        ...
        public TextRecognizerImplementation(IInputSource inputSource) : base(inputSource)
        {
            m_instance = new NativeGoogleMLKitTextRecognizer(NativeUnityPluginUtility.GetContext(), GetNativeInputImageProducer(inputSource));
        }

        public void Prepare(TextRecognizerOptions options, OnPrepareCompleteInternalCallback callback)
        {
            if (!m_initialised)
            {
                Initialise();
